...
namespace werkzeug
{
	namespace detail
	{
		...
		template <typename T>
		using difference_trait = trait_base<requires( T t ) {
			{ t - t } -> std::convertible_to<std::ptrdiff_t>;
		}, false, requires( T t ) {
			{ t - t } noexcept;
		}>;
	...
	template <typename Fct, typename Base, typename... Children>
	concept has_common_invoke_result = requires { typename common_invoke_result<Fct, Base, Children...>::type; };

	template <typename Fct, typename Base, typename... Children>
	struct common_result_type_or_void
	{
		using type = typename std::conditional_t< has_common_invoke_result<Fct, Base, Children...>, common_invoke_result<Fct, Base, Children...>, std::type_identity<void> >::type;
	};
	...
	struct is_instantiation_of<Template<Ts...>, Template> : std::true_type
	{
	};

	template <typename Type, template <typename...> class Template>
	concept instantiation_of = is_instantiation_of<Type, Template>::value;
} // namespace werkzeug
