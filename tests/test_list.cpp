...
TEST_CASE("list")
{
	test_resource resource;
	{
		...
		REQUIRE( Lifetime_Informer::stats().instance_counter == 2 );
	...
	REQUIRE( Lifetime_Informer::stats().dtor_counter == 3 );
...

	{
		...
		l1.emplace_back( 1 );
		l1.emplace_back( 2 );
		REQUIRE( Lifetime_Informer::stats().instance_counter == 3 );
		REQUIRE( Lifetime_Informer::stats().dtor_counter == 0 );

		list l2{ l1 };
		REQUIRE( l2.size() == 3 );
		REQUIRE( Lifetime_Informer::stats().copy_ctor_counter == 3 );
		REQUIRE( Lifetime_Informer::stats().instance_counter == 6 );
	...
	REQUIRE( Lifetime_Informer::stats().dtor_counter == 6 );
	REQUIRE( resource.stats().delta() == 0 );
}